...
import { AccesibilityNames, IMAGE_BASE_URL, LANGUAGE_DROPDOWN_LIST, PROFILE_DROPDOWN_LIST} from '@config/constant';
		...
		<header className='bg-white z-10 h-14 border-b border-b-color-4 flex'>
			<nav className=' flex items-center w-full' aria-label='Global'>
				...
				<Link to='/' className='w-wide-7 h-14 flex flex-1 items-center lg:flex-initial justify-end lg:justify-center'>
					...
					{!toggleHeaderImage ? <img className='h-10 w-wide-10 object-contain ' src={logo || defaultLogoLarge} alt='LogoImage' /> : <img className='h-rise-3 w-wide-2 object-contain ' src={logo || defaultLogoSmall} alt='LogoImage' />}
				</Link>
