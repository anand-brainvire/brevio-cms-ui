...
const AddEditCategory = (): ReactElement => {
  ...
  useEffect(() => {
    if (faqByIdData && params.id) {
      ...
      const translation = Array.isArray(data?.category_translations)
        ? data.category_translations.find(
            (tr: { lang_code: string }) => tr.lang_code === 'en'
          ) || data.category_translations[0]
        : {};

      formik
        .setValues({
          goalId: Array.isArray(data?.goals)
            ? data.goals.map((g: { uuid: string }) => g.uuid)
            : [],
        ...
        updateFaq({
          ...
          .then((res) => {
            ...
            if (data?.updateCategory?.meta?.statusCode === 200) {
              ...
              toast.error(
                data?.updateCategory?.meta?.message || t('Update failed')
              );
            }
          })
          .catch(() => {
  ...
  const handleViewAllBooks = useCallback(() => {
    if (params.id) {
      navigate(
        `/${ROUTES.app}/${ROUTES.manageBooks}/${ROUTES.list}?categoryId=${params.id}`
      );
    }
  }, [params.id, navigate]);
      ...
      <form onSubmit={formik.handleSubmit}>
        <div className='card-body'>
          <div className='card-title-container'>
            <p>
              {t('Fields marked with')} <span className='text-red-700'>*</span>{' '}
              {t('are mandatory.')}
            </p>
          </div>
          <div className='card-grid-addedit-page'>
            ...
            <div>
              <TextInput
                ...
                type='text'
                id='categorySlug'
                placeholder={t('Add Slug')}
                name='categorySlug'
                onChange={formik.handleChange}
                value={formik.values.categorySlug}
                error={getErrorFaq('categorySlug')}
              />
            </div>
            <div>
        ...
        <div className='card-footer btn-group'>
          <Button
            ...
            type='submit'
            label={params.id ? t('Update') : t('Save')}
          >
        ...
        {params.id && formik.values.status === STATUS.active && (
          <div className='card-footer-secondary btn-group'>
            <Button
              className='btn-secondary '
              onClick={handleViewAllBooks}
              label={t('View All Books')}
            >
              <span className='text-white mr-1 w-3.5 h-3.5 inline-block svg-icon'>
                <CheckCircle />
              </span>
